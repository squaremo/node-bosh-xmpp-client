...
function nxbClient(jid, password, bosh, route)
{
	...
	this.sendHttp = function(body)
	{
		var that = this;
		this.chold++;
		autil.xmlHttpRequest(this.options, function(err, response) {that.handle(err, response)}, body, this.use_ssl);
	}
		...
		if(this.state == STATE_BIND)
		{
			...
			if(iq = body.getChild("iq", NS_CLIENT))
			{
				if(iq.attrs.id == "bind_1" && iq.attrs.type == "result")
				{
		            ...
		            this.sess_attr.jid = autil.jid_parse(cjid);
	...
	this.startSasl = function(features)
	{
		...
		for(i = 0;i<mechanisms.children.length;i++)
		{
			if(mechanisms.children[i].getText() == "PLAIN")
			{
				...
				this.sendXml(e);
				return;
			} else if (mechanisms.children[i].getText() == 'ANONYMOUS') {
				var e = new ltx.Element("auth", {xmlns : NS_XMPP_SASL, mechanism : "ANONYMOUS"});
				this.sendXml(e);
                return;
            }
		}
